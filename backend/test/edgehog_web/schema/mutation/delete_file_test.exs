#
# This file is part of Edgehog.
#
# Copyright 2026 SECO Mind Srl
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#    http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# SPDX-License-Identifier: Apache-2.0
#

defmodule EdgehogWeb.Schema.Mutation.DeleteFileTest do
  use EdgehogWeb.GraphqlCase, async: true

  import Edgehog.FilesFixtures

  alias Edgehog.Files.File
  alias Edgehog.Files.File.BucketStorage, as: Storage

  require Ash.Query

  describe "deleteFile mutation" do
    setup %{tenant: tenant} do
      file =
        file_fixture(tenant: tenant)

      id = AshGraphql.Resource.encode_relay_id(file)

      %{file_fixture: file, id: id}
    end

    test "deletes existing base image", %{tenant: tenant, id: id, file_fixture: fixture} do
      expect(Storage, :delete, 3, fn file, encoding ->
        assert file.id == fixture.id

        case encoding do
          nil ->
            :ok

          :gz ->
            :ok

          :lz4 ->
            :ok
        end
      end)

      file =
        [tenant: tenant, id: id]
        |> delete_file_mutation()
        |> extract_result!()

      assert file["name"] == fixture.name

      refute File
             |> Ash.Query.filter(id == ^fixture.id)
             |> Ash.Query.set_tenant(tenant)
             |> Ash.exists?()
    end

    test "tries to delete the file, but ignores failure", %{tenant: tenant} do
      fixture = file_fixture(tenant: tenant)

      id = AshGraphql.Resource.encode_relay_id(fixture)

      expect(Storage, :delete, 3, fn file, encoding ->
        assert file.id == fixture.id

        case encoding do
          nil -> {:error, :cannot_delete}
          :gz -> {:error, :cannot_delete}
          :lz4 -> {:error, :cannot_delete}
        end
      end)

      %{"id" => ^id} =
        [tenant: tenant, id: id]
        |> delete_file_mutation()
        |> extract_result!()
    end

    test "fails with non-existing file", %{tenant: tenant, file_fixture: file, id: id} do
      :ok = Ash.destroy!(file, action: :destroy_fixture)

      result = delete_file_mutation(tenant: tenant, id: id)

      assert %{fields: [:id], message: "could not be found"} = extract_error!(result)
    end
  end

  defp delete_file_mutation(opts) do
    default_document = """
    mutation DeleteFile($id: ID!) {
      deleteFile(id: $id) {
        result {
          id
          name
          baseFile {
            url
          }
          gzFile {
            url
          }
          lz4File {
            url
          }
        }
      }
    }
    """

    {tenant, opts} = Keyword.pop!(opts, :tenant)
    {id, opts} = Keyword.pop!(opts, :id)

    document = Keyword.get(opts, :document, default_document)
    variables = %{"id" => id}
    context = %{tenant: tenant}

    Absinthe.run!(document, EdgehogWeb.Schema, variables: variables, context: context)
  end

  defp extract_error!(result) do
    assert %{
             data: %{"deleteFile" => nil},
             errors: [error]
           } = result

    error
  end

  defp extract_result!(result) do
    refute :errors in Map.keys(result)
    refute "errors" in Map.keys(result[:data])

    assert %{
             data: %{
               "deleteFile" => %{
                 "result" => file
               }
             }
           } = result

    assert file

    file
  end
end
