...
defmodule Edgehog.Campaigns.CampaignMechanism.DeploymentUpgradeCoreTest do
  @moduledoc false
  ...
  describe "pending_request_timeout_ms/3" do
    ...
    test "raises if latest_attempt is not set", ctx do
      campaign_mechanism = deployment_upgrade_mechanism_fixture()
    ...
    test "returns the remaining milliseconds if the timeout is not expired", ctx do
      ...
      campaign_mechanism = deployment_upgrade_mechanism_fixture(request_timeout_seconds: 5)
  ...
  test "increase_retry_count!/1", %{tenant: tenant} do
    target = target_fixture(tenant: tenant)
    new_target = MechanismCore.increase_retry_count!(%DeploymentUpgrade{}, target)
  ...
  describe "can_retry?/2" do
    ...
    test "returns true if the target still has retries left", ctx do
      ...
      campaign_mechanism = deployment_upgrade_mechanism_fixture(request_retries: 5)
  ...
  describe "get_target!/2" do
    test "returns target if existing", %{tenant: tenant} do
      ...
      assert %CampaignTarget{id: ^target_id} =
               MechanismCore.get_target!(%DeploymentUpgrade{}, tenant.tenant_id, target_id)
    end

    test "raises with non-existing target", %{tenant: tenant} do
      assert_raise Invalid, fn ->
        MechanismCore.get_target!(%DeploymentUpgrade{}, tenant.tenant_id, 1_234_567)
      end
    end
  end
  ...
  describe "available_slots/2" do
    test "returns the number of available update slots given the current in progress count" do
      mechanism = deployment_upgrade_mechanism_fixture(max_in_progress_operations: 10)
      in_progress = 7
    ...
    test "returns 0 if there are more in progress updates than allowed" do
      mechanism = deployment_upgrade_mechanism_fixture(max_in_progress_operations: 5)
      in_progress = 7
  ...
  test "mark_target_as_failed!/2", %{tenant: tenant} do
    ...
    target =
      MechanismCore.mark_target_as_failed!(%DeploymentUpgrade{}, target, completion_timestamp)
  ...
  test "mark_target_as_successful!/2", %{tenant: tenant} do
    ...
    target =
      MechanismCore.mark_target_as_successful!(%DeploymentUpgrade{}, target, completion_timestamp)
  ...
  describe "error_message/3" do
    ...
    test "returns specific error message for known errors", ctx do
      ...
      for error <- known_errors do
        msg = MechanismCore.error_message(%DeploymentUpgrade{}, error, ctx.device_id)
    ...
    test "returns generic error message for unknown error", ctx do
      msg = MechanismCore.error_message(%DeploymentUpgrade{}, :a_new_kind_of_error, ctx.device_id)
      assert msg =~ ctx.device_id
  ...
  describe "temporary_error?/1" do
    test "returns true for connection refused" do
      assert MechanismCore.temporary_error?(%DeploymentUpgrade{}, "connection refused") == true
    end

    test "returns true for API errors with status code in 500..599" do
      for status <- 500..599 do
        assert MechanismCore.temporary_error?(%DeploymentUpgrade{}, %APIError{
                 status: status,
                 response: "Error"
               }) ===
                 true
    ...
    test "returns false for known non temporary errors" do
      ...
      for error <- known_non_temporary_errors do
        assert MechanismCore.temporary_error?(%DeploymentUpgrade{}, error) == false
      end
    end

    test "returns false for unknown errors" do
      assert MechanismCore.temporary_error?(%DeploymentUpgrade{}, :a_new_kind_of_error) == false
    end
  end

  test "get_target_count/2", %{tenant: tenant} do
    campaign =
      campaign_with_targets_fixture(42, tenant: tenant, mechanism_type: :deployment_upgrade)

    assert MechanismCore.get_target_count(%DeploymentUpgrade{}, tenant.tenant_id, campaign.id) ==
             42
  end
  ...
  test "get_failed_target_count/2", %{tenant: tenant} do
    ...
    |> Enum.take(7)
    |> Enum.each(fn target ->
      MechanismCore.mark_target_as_failed!(%DeploymentUpgrade{}, target)
    end)
  ...
  test "get_in_progress_target_count/2", %{tenant: tenant} do
    campaign =
      ...
      |> Ash.load!(
        ...
        campaign_mechanism: [deployment_upgrade: [:release]]
      )
  ...
  describe "has_idle_targets?/2" do
    test "returns true for campaigns with a least one idle target", %{tenant: tenant} do
      ...
      |> Enum.take(4)
      |> Enum.each(fn target ->
        MechanismCore.mark_target_as_successful!(%DeploymentUpgrade{}, target)
      end)

      assert MechanismCore.has_idle_targets?(%DeploymentUpgrade{}, tenant.tenant_id, campaign.id) ==
               true
    end
  ...
  test "mark_campaign_in_progress!/1", %{tenant: tenant} do
    ...
    campaign =
      campaign_with_targets_fixture(3, tenant: tenant, mechanism_type: :deployment_upgrade)

    assert %Campaign{status: :in_progress, start_timestamp: ^now} =
             MechanismCore.mark_campaign_in_progress!(%DeploymentUpgrade{}, campaign, now)
  end
  ...
  test "mark_campaign_as_failed!/1", %{tenant: tenant} do
    ...
    assert %Campaign{status: :finished, outcome: :failure, completion_timestamp: ^now} =
             MechanismCore.mark_campaign_as_failed!(%DeploymentUpgrade{}, campaign, now)
  end
  ...
  test "mark_campaign_as_successful!/1", %{tenant: tenant} do
    ...
    assert %Campaign{status: :finished, outcome: :success, completion_timestamp: ^now} =
             MechanismCore.mark_campaign_as_successful!(%DeploymentUpgrade{}, campaign, now)
  end
  ...
  defp deployment_upgrade_mechanism_fixture(attrs \\ []) do
    attrs
    |> Enum.into(%{
    ...
    |> then(&struct!(DeploymentUpgrade, &1))
  end
  ...
  defp set_target_retry_count!(target, count) do
    ...
    Enum.reduce(1..count, target, fn _idx, target ->
      MechanismCore.increase_retry_count!(%DeploymentUpgrade{}, target)
    end)
  end
