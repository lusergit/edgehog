...
defmodule Edgehog.Campaigns.Executors.DeploymentDeployExecutorTest do
  ...
  describe "Executor receiving a Deployment update" do
    ...
    for status <- [:started, :stopped] do
      test "frees up slot if Deployment state is #{status}", ctx do
        ...
        tenant
        |> update_deployment_state!(deployment_id, unquote(status))
        |> broadcast_readiness()
  ...
  describe "Executor marks deployment deploy campaign as successful" do
    ...
    test "if all targets are successful", ctx do
      ...
      tenant
      |> mark_all_pending_deployments_with_state(campaign_id, :stopped)
    ...
    test "if just less than `max_failure_percentage` targets fail", ctx do
      ...
      Enum.each(successful_deployment_ids, fn id ->
        tenant
        |> update_deployment_state!(id, :stopped)
        |> broadcast_readiness()
      end)
  ...
  describe "Executor marks campaign as failed if `max_failure_percentage` is exceeded" do
    ...
    test "by failed Deployments", ctx do
      ...
      Enum.each(remaining_successful_targets, fn target ->
        tenant
        |> update_deployment_state!(target.deployment_id, :stopped)
        |> broadcast_readiness()
      end)
    ...
    # TODO: Skip this until a flow for failures is established.
    @tag :skip
