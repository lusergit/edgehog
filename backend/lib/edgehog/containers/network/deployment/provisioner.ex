...
defmodule Edgehog.Containers.Network.Deployment.Provisioner do
  ...
  Each and every time an network should be deployed, it can be done trough
  this provisioner. The provisioner sends the appropriate messages to the device
  and emits a `ready:network_deployment:id` event whenever network
  is present in the device.

  The provisioning flow can be described as follows:

  - start_link/1 called, init the process
  - The server subscribes to events on 'network_deployments:id' (id of the network deployment)
  ...
  - Astarte triggers update the network deployment state, marking it as present or not present and emitting an event on the correct topic
  - The server reacts to the event, handles the message and emits an event on 'ready:network_deployment:id' when the resource is ready
  ...
  - A :timeout hits the server, it retries to send the network information to the device
  ...
  def handle_continue(
        ...
        %{network_deployment: network_deployment} = state
      ) do
    network_deployment =
  ...
  def handle_continue(:send, old_state) do
    ...
    sent = Core.send(network_deployment, tenant: tenant, deployment: deployment)
  ...
  # We get the network deployment from the broadcast, which is in the
  # :payload -> :data section. This network deployment is more recent, as
  ...
  defp retry_or_stop(error, state) do
    ...
    if retries < max_retries do
      ...
      Logger.error("""
      An error occurred while sending the network deployment #{id}:
      #{inspect(error)}
