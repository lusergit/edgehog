...
defmodule Edgehog.Containers.Container.Changes.Relate do
  ...
  def change(changeset, _opts, %{tenant: tenant}) do
    with {:ok, container} <- Ash.Changeset.fetch_argument(changeset, :container),
         ...
         {:ok, container} <- Ash.load(container, [:image, :volumes, :networks, :device_mappings], tenant: tenant) do
      ...
      networks_input =
        Enum.map(networks, fn network ->
          %{network: network, device: device, deployment: deployment, network_id: network.id, device_id: device.id}
        end)

      volumes_input =
        Enum.map(volumes, fn volume ->
          %{volume: volume, device: device, deployment: deployment, volume_id: volume.id, device_id: device.id}
        end)

      device_mappings_input =
        Enum.map(device_mappings, fn device_mapping ->
          %{device_mapping: device_mapping, device: device, deployment: deployment, device_mapping_id: device_mapping.id, device_id: device.id}
        end)
