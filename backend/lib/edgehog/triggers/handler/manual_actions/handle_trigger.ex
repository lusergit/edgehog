...
defmodule Edgehog.Triggers.Handler.ManualActions.HandleTrigger do
  ...
  defp handle_event(%IncomingData{interface: @available_images} = event, tenant, _realm_id, device_id, _timestamp) do
    case String.split(event.path, "/") do
      ["", image_id, "pulled"] ->
        with {:ok, deployment} <-
               Containers.fetch_image_deployment(image_id, device_id, tenant: tenant) do
          if event.value do
            Containers.image_deployment_pulled(deployment)
          else
            Containers.image_deployment_unpulled(deployment)
          end
        end
  ...
  defp handle_event(%IncomingData{interface: @available_networks} = event, tenant, _realm_id, device_id, _timestamp) do
    case String.split(event.path, "/") do
      ["", network_id, "created"] ->
        with {:ok, deployment} <-
               Containers.fetch_network_deployment(network_id, device_id, tenant: tenant) do
          if event.value do
            Containers.network_deployment_available(deployment)
          else
            Containers.network_deployment_unavailable(deployment)
          end
        end
  ...
  defp handle_event(%IncomingData{interface: @available_containers} = event, tenant, _realm_id, device_id, _timestamp) do
    case String.split(event.path, "/") do
      ["", container_id, "status"] ->
        with {:ok, deployment} <-
               Containers.fetch_container_deployment(container_id, device_id, tenant: tenant) do
          case event.value do
            "Received" -> Containers.container_deployment_received(deployment)
            "Created" -> Containers.container_deployment_created(deployment)
            "Stopped" -> Containers.container_deployment_stopped(deployment)
            "Running" -> Containers.container_deployment_running(deployment)
          end
        end
  ...
  defp handle_event(%IncomingData{interface: @available_deployments} = event, tenant, _realm_id, _device_id, _timestamp) do
    case String.split(event.path, "/") do
      ["", deployment_id, "status"] ->
        with {:ok, deployment} <- Ash.get(Release.Deployment, deployment_id, tenant: tenant) do
          case event.value do
            "Started" ->
              Containers.release_deployment_started(deployment)

            "Stopped" ->
              Containers.release_deployment_stopped(deployment)
